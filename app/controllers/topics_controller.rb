...
class TopicsController < ApplicationController
  ...
  def search
    ...
    results = Topic.includes(:user).where("LOWER(title) LIKE ?", "%#{term.downcase}%")
    results = results.map{|c| {title: c.title, created_by: c.user.name,  url: c.url, created_ago: time_ago_in_words(c.created_at - 1) }}
    to_render = { topics: results.as_json }
